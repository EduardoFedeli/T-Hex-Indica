...
export default function SupabaseProgressBar() {
  ...
  useEffect(() => {
    async function fetchDbSize() {
      // Chama a função RPC que criamos lá no SQL Editor do Supabase
      const { data, error } = await supabase.rpc('get_db_size')
      
      if (data && !error) {
        // Converte o valor de Bytes para Megabytes
  ...
  // Calcula a porcentagem para preencher a barrinha verde
